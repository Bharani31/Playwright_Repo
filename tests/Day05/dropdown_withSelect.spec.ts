...
test("Launch a Browser : ", async( {page} ) => {
    ...
    await page.selectOption(`//select[@id="createLeadForm_industryEnumId"]`, {value:"IND_SOFTWARE"})

    const allIndustryValue = page.locator(`//select[@id="createLeadForm_industryEnumId"]/option`)
    const count = await allIndustryValue.count()

    for (let i = 0; i < count; i++) {

        const text = await allIndustryValue.nth(i).textContent();
